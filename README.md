...
- `schedule_video_batch`: schedule a finite manual batch of 1-30 videos (one topic per video; each provider needs at least one connected account — the batch targets all connected accounts of each provider; tokens are charged upfront for the whole batch).
