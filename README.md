...
- `schedule_video_batch`: schedule a finite manual batch of videos (one topic per video; each provider needs at least one connected account — the batch targets all connected accounts of each provider; tokens are charged upfront for the whole batch).
